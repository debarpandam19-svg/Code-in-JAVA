...
* **`ArrayMaxFinder.java`** – A program that takes user input to create an array and finds the largest element using a separate method.
...
* Working with arrays and traversing them using loops.
...
*(Replace `ATM` with `BattleGame`, `GameIdeaSimulation`, or `ArrayMaxFinder` to run the other programs.)*
